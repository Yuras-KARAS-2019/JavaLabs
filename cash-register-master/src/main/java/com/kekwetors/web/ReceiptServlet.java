...
public class ReceiptServlet extends AbstractBaseServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) {
        ...
        if (id != null) {
            ...
            if (isFrontend != null) {
                req.setAttribute("receipts",
                        receiptDao.getAllReceipts().stream().map(receiptConverter::toDto).collect(Collectors.toList()));

                RequestDispatcher dispatcher = this.getServletContext().getRequestDispatcher("/WEB-INF/views/receiptsView.jsp");
